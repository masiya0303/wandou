...
export function retrieveRelevantMemories(
  ...
  if (store.memories.length === 0) return { entries: [], contextText: '' }

  const queryTokens = tokenize(queryText)
  if (queryTokens.length === 0) return { entries: [], contextText: '' }

  // 计算每条记忆的匹配分数
  interface Scored { entry: MemoryEntry; score: number }
  const scored: Scored[] = []
  ...
  for (const mem of store.memories) {
    ...
    const memTokens = tokenize(searchText)

    const similarity = jaccardSimilarity(queryTokens, memTokens)

    // 关键词匹配加权
    let keywordHit = 0
    for (const kw of mem.keywords) {
      if (queryText.toLowerCase().includes(kw.toLowerCase())) keywordHit++
    }
    for (const ent of mem.entities) {
      if (queryText.toLowerCase().includes(ent.toLowerCase())) keywordHit++
    }

    const score = similarity * 0.5 + (keywordHit > 0 ? Math.min(1, keywordHit * 0.3) : 0) + mem.importance * 0.1
    if (score > 0.05) {
      scored.push({ entry: mem, score })
    }
  }
  ...
  // 构建上下文文本
  if (selected.length === 0) return { entries: [], contextText: '' }

  const lines = ['【相关历史事件】']
  for (const mem of selected) {
    const catLabel = CATEGORY_LABELS[mem.category] || mem.category
    lines.push(`- [${catLabel}] ${mem.fact}`)
  }
  lines.push('')
