...
<template>
  <div class="input-area">
    <div class="input-row">
      <textarea ref="ta" v-model="input" class="input" placeholder="输入行动或对话..." rows="1" :disabled="store.isGenerating" @keydown="onKey" @input="autoResize"></textarea>
      <button v-if="store.isGenerating" class="stop-btn" @click="store.stopGeneration()">⏹ 停止</button>
...
.input-row {
  ...
  background: var(--pink-input-bg);
  border: 4px solid var(--pink-ice);
  border-left: none; border-top: none;
  border-right: 4px solid var(--pink-light);
...
.input {
  ...
  color: var(--pink-primary); font-size: 14px; font-family: inherit;
...
.send-btn {
  ...
  background: var(--pink-light); color: #fff; border: none; border-radius: 10px;
  cursor: pointer; font-size: 13px; font-weight: 600; font-family: inherit;
  transition: all 0.2s;
}
.send-btn:active:not(:disabled) { background: var(--pink-accent); transform: scale(0.96); }
.send-btn:disabled { background: var(--pink-ice); color: rgba(255,255,255,0.5); cursor: not-allowed; }
