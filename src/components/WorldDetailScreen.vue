  ...
  <div class="screen">
    ...
    <div class="container">
      ...
      <header class="top">
        ...
        <h1>🪐 {{ store.worldName }}</h1>
        <button class="btn-save glass-panel" @click="saveWorldInfo">💾 {{ saved ? '已保存' : '保存' }}</button>
      </header>
      ...
      <div class="sec glass-panel corner-deco">
        ...
        <textarea v-model="editDesc" class="fi ta" rows="3" placeholder="世界描述..."></textarea>
      </div>
      ...
      <!-- 底部操作 -->
      <div class="bottom">
        <button class="btn-enter corner-deco" @click="handleEnterGame">
          <span class="cn">🚀 进入冒险</span>
          <span class="en">ENTER WORLD</span>
        </button>
...
.ta { resize: vertical; min-height: 50px; }
...
.entry { padding: 0.4rem 0.5rem; border-radius: 6px; background: rgba(8,16,28,0.25); margin-bottom: 0.3rem; transition: all 0.15s; }
...
.btn-enter:hover { background: rgba(0,229,255,0.18); box-shadow: 0 0 20px var(--accent-cyan-glow); transform: translateY(-2px); }
