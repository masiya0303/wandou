...
async function handleEnter(id: string) { const ok = await store.openWorldDetailFromList(id); if (ok && store.character.name) store.phase = 'playing' }
async function handleDelete(id: string, name: string) { if (!confirm(`确定删除世界「${name}」？`)) return; await store.deleteWorld(id) }
async function handleCreate() { await store.createWorld('新世界', ''); store.phase = 'worldDetail' }
...
<template>
  <div class="screen">
    ...
    <div class="container">
      ...
      <div v-if="store.worldList.length === 0" class="empty"><p>🌌 暂无世界</p><p class="hint">点击上方「创建」开始</p></div>
      <div v-else class="list">
        <div v-for="w in store.worldList" :key="w.id" class="card" @click="handleEnter(w.id)">
...
.title p { font-size: 10px; color: var(--pink-light); letter-spacing: 0.15em; margin: 2px 0 0; }
.btn-back { padding: 6px 14px; border-radius: 8px; border: 1px solid var(--pink-ice); background: rgba(255,255,255,0.6); color: var(--pink-primary); font-size: 13px; cursor: pointer; font-family: inherit; }
.btn-back:active { background: var(--pink-ice); }
.btn-create { padding: 6px 14px; border-radius: 8px; border: 1px solid var(--pink-accent); background: rgba(255,255,255,0.6); color: var(--pink-accent); font-size: 13px; cursor: pointer; font-family: inherit; }
.btn-create:active { background: var(--pink-ice); }
.empty { text-align: center; padding: 48px 16px; color: var(--pink-primary); opacity: 0.6; }
.empty .hint { font-size: 12px; opacity: 0.6; }
.list { display: flex; flex-direction: column; gap: 12px; }
.card { padding: 14px 16px; border-radius: 16px; cursor: pointer; transition: all 0.25s; background: rgba(255,255,255,0.7); border: 1px solid var(--pink-ice); }
.card:active { border-color: var(--pink-accent); transform: translateY(-2px); box-shadow: 0 0 15px rgba(255,128,168,0.15); }
...
.c-name { font-size: 16px; font-weight: 600; color: var(--pink-primary); }
.c-meta { font-size: 11px; color: var(--pink-primary); opacity: 0.55; }
.c-time { font-size: 10px; color: var(--pink-primary); opacity: 0.4; }
.c-desc { font-size: 12px; color: var(--pink-primary); opacity: 0.6; margin: 6px 0 0; line-height: 1.4; }
.c-acts { display: flex; gap: 6px; margin-top: 8px; justify-content: flex-end; }
.a-enter { padding: 4px 10px; border-radius: 6px; background: rgba(255,128,168,0.08); border: 1px solid rgba(255,128,168,0.25); color: var(--pink-accent); font-size: 12px; cursor: pointer; font-family: inherit; }
