...
// 子页面导航：从 route param 或本地状态
const page = ref<string | null>(null)
// 同步路由 param
const sectionParam = computed(() => route.params.section as string | undefined)
if (sectionParam.value) page.value = sectionParam.value
  ...
  <div class="ss">
    ...
    <div v-if="page" class="sub">
      <header class="sub-head">
        ...
        <h2>{{ {api:'🔌 API 配置',prompt:'📜 系统提示词',worldbook:'📖 全局世界书',extensions:'🧩 扩展管理',theme:'🎨 聊天主题'}[page] }}</h2>
        <span></span>
      </header>
      <div class="sub-body">
        ...
        <!-- Extensions -->
        <div v-if="page === 'extensions'" class="form-card glass-panel">
...
.sub-head { display: flex; align-items: center; justify-content: space-between; padding: 12px 0; flex-shrink: 0; border-bottom: 1px solid var(--theme-border-ice); }
.sub-head h2 { font-size: 18px; color: #e0e8ff; margin: 0; }
.sub-back { padding: 6px 14px; border-radius: 20px; border: 1px solid var(--theme-border-ice); background: rgba(255,255,255,0.5); color: var(--theme-text-main); font-size: 12px; cursor: pointer; font-family: inherit; }
.sub-back:active { background: var(--theme-border-ice); }
