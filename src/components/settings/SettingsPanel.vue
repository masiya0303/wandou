...
const EXPAND_TITLES: Record<string, string> = { api:'🔌 API 配置', prompt:'📜 系统提示词', worldbook:'📖 世界书', extensions:'🧩 扩展管理', theme:'🎨 聊天主题' }
</script>
...
<template>
  <div class="ss">
    ...
    <div v-if="expandedKey" class="ss-expand">
      ...
      <!-- 世界书：直接撑满剩余空间，不经过滚动容器 -->
      <div v-if="expandedKey === 'worldbook'" class="ex-fill">
        <WorldBookManager />
      </div>
...
/* ---- home ---- */
.ss-main { position: relative; z-index: 1; display: flex; flex-direction: column; flex: 1; min-height: 0; padding: 16px; }
.ss-head { display: grid; grid-template-columns: 1fr auto 1fr; align-items: center; padding: 8px 0 4px; flex-shrink: 0; }
...
.ss-close { width: 36px; height: 36px; border-radius: 50%; font-size: 16px; color: var(--theme-text-main); cursor: pointer; border: 1px solid var(--theme-border-ice); background: rgba(255,255,255,0.5); display: flex; align-items: center; justify-content: center; justify-self: end; margin-right: 4px; }
...
/* ---- expanded detail ---- */
.ss-expand { position: relative; z-index: 1; display: flex; flex-direction: column; flex: 1; min-height: 0; padding: 0 16px 16px; overflow: hidden; animation: slideRightIn 0.2s ease-out; }
@keyframes slideRightIn { from { opacity: 0; transform: translateX(20px); } to { opacity: 1; transform: translateX(0); } }
.ex-head { display: grid; grid-template-columns: 1fr auto 1fr; align-items: center; padding: 12px 0; border-bottom: 1px solid var(--theme-border-ice); flex-shrink: 0; }
...
/* worldbook 专用 */
.ex-fill { flex: 1; min-height: 0; position: relative; padding: 0; }

/* 其他卡片 — 可滚动 */
.ex-body { flex: 1; min-height: 0; overflow-y: auto; padding: 16px 0; max-width: 540px; width: 100%; margin: 0 auto; }
